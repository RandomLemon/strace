...
decode_loop_info(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 tprints(", lo_name=");
 print_quoted_string(info.lo_name, 64,
       0x01);
 if (!0 || info.lo_encrypt_type != 0) {
  tprints(", lo_encrypt_key=");
  print_quoted_string((void *) info.lo_encrypt_key,
        ((((uint32_t) info.lo_encrypt_key_size) < (32)) ? ((uint32_t) info.lo_encrypt_key_size) : (32)), 0);
 }
 if (!0)
...
decode_loop_info64(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 if (!0 || info64.lo_encrypt_type != 0) {
  ...
  print_quoted_string((void *) info64.lo_encrypt_key,
        (((info64.lo_encrypt_key_size) < (32)) ? (info64.lo_encrypt_key_size) : (32)), 0);
 }
 if (!0)
  tprintf(", lo_init=[%#" "l" "x" ", %#" "l" "x" "]}",
